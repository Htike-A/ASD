...
class DateScrollbar:
    def __init__(self):
        ...
        # Create canvas and scrollbar in the container
        self.canvas = tk.Canvas(container, height=60)
        self.scrollbar = ttk.Scrollbar(container, orient="horizontal", command=self.canvas.xview)
        
        # Attach scrollbar to canvas (not to the main frame)
        self.canvas.configure(xscrollcommand=self.scrollbar.set)
        
        # Position the canvas and scrollbar
        self.canvas.grid(row=0, column=0, sticky="ew")  # Using grid for better control
